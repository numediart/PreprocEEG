function [X_imf] = EEMD_decompose(X,NE,NoiseLevel,fs,numImf);
% program  by  Xueyuan Xu and Luchang Li.
% EEMD_decompose generates a IMF matrix Y from the original series X
...
X_imf = [];
        ...
        allmode(Residue,:) = R(nch,:);
        X_imfnch = allmode;
        X_imf = cat(1,X_imf,X_imfnch);
        end
end
