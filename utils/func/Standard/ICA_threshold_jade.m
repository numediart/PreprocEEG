function [Y,W] = ICA_threshold_jade(Sica,WC,B,P_ica,acthreshold1,acthreshold2,enthreshold1,enthreshold2,kuthreshold1,kuthreshold2,vathreshold1,vathreshold2)
% This is the threshold selection program by  Wei Tao and Luchang Li.
%  [Y,W]=ICA_threshold_jade(Sica,WC,Wica,P_ica,acthreshold1,acthreshold2,enthreshold1,enthreshold2,kuthreshold1,kuthreshold2)
%  produces a matrix Y of dimension [m by T].
%  which is an estimate of clean signals processed by the jade algorithm.
...
for cv = 1:Lend
    if variances(cv,:) < 1
      Sica(cv,:) = 0;
             ...
             Y = inv(WC{1,1}')*inv(B)*(inv(P_ica)*Sica);
             W = B;
end
